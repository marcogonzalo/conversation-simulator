#!/usr/bin/env python3
"""
Integration test for voice conversation.
...
class VoiceConversationTester:
    ...
    async def setup(self):
        ...
        self.conversation_service = ConversationApplicationService(conversation_repo, conversation_domain_service)
    ...
    async def test_websocket_connection(self, conversation_id: str):
        ...
        logger.info(f"Testing WebSocket connection for conversation {conversation_id}...")
    ...
    async def test_persona_loading(self):
        ...
        try:
            ...
            if persona:
                logger.info(f"Persona loaded: {persona.name} ({persona.accent.value})")
                return True
            else:
    ...
    async def run_tests(self):
        ...
        logger.info("All tests passed! Voice conversation integration is working.")
        return True
...
async def main():
    ...
    if success:
        logger.info("🎉 Voice conversation integration test completed successfully!")
        return True
    else:
